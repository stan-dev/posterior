...
[`as_draws()`](https://mc-stan.org/posterior/reference/draws.md), which
attempts to find the closest available format to the input object. In
this case the result would be a `draws_matrix` object either way.
...
see
[`help("diagnostics", "posterior")`](https://mc-stan.org/posterior/reference/diagnostics.md).
