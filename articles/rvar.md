...
[`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) datatype, a
...
The [`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) datatype
is inspired by the [rv](https://cran.r-project.org/package=rv) package
and [Kerman and Gelman
...
The [`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) datatype
is a wrapper around a multidimensional array where the first dimension
is the number of draws in the random variable. The most direct way to
create a random variable is to pass such an array to the
[`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) function.
...
[`match()`](https://mc-stan.org/posterior/reference/match.md) and
...
[`vignette("posterior")`](https://mc-stan.org/posterior/articles/posterior.md)
...
The [`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) datatype
implements most math operations, including basic arithmetic, functions
in the *Math* and *Summary* groups, like
...
[`as_rvar()`](https://mc-stan.org/posterior/reference/as_rvar.md), which
will return an `rvar` with one draw and the same dimensions as its
input:
...
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md), executing
expressions of random variables with
[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md), or evaluating
random number generator functions using
[`rvar_rng()`](https://mc-stan.org/posterior/reference/rvar_rng.md).

### Converting functions with `rfun()`

The [`rfun()`](https://mc-stan.org/posterior/reference/rfun.md) wrapper
converts an existing R function into a new function that `rvar`s can be
passed to it as arguments, and which will return `rvar`s. We can use
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md) to convert
the base [`rnorm()`](https://rdrr.io/r/stats/Normal.html) and
...
While
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md)-converted
...
An alternative to
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md) is to use
[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md), which can be
passed nearly-arbitrary R expressions. The expression will be executed
multiple times to construct an `rvar`. E.g., we can write an expression
...
[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md) expressions
can also contain other `rvar`s, so long as all `rvar`s in the expression
have the same number of draws. Thus, we can re-write the example above
that used [`rfun()`](https://mc-stan.org/posterior/reference/rfun.md) as
follows:
...
Like [`rfun()`](https://mc-stan.org/posterior/reference/rfun.md),
[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md) is not
necessarily fast, so you may find it more useful for prototyping than
production code.

### Evaluating random number generators with `rvar_rng()`

[`rvar_rng()`](https://mc-stan.org/posterior/reference/rvar_rng.md) is
an alternative to
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md)/[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md)
...
[`rvar_rng()`](https://mc-stan.org/posterior/reference/rvar_rng.md), and
it will adapt it to be able to take `rvar` arguments and return an
`rvar`, as follows:
...
In contrast to the
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md) and
[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md) examples
above,
[`rvar_rng()`](https://mc-stan.org/posterior/reference/rvar_rng.md)
...
[`help("rvar-slice")`](https://mc-stan.org/posterior/reference/rvar-slice.md).
