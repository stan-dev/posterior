...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md) which are of
size one. See [`base::drop()`](https://rdrr.io/r/base/drop.html)
...
An [`rvar`](https://mc-stan.org/posterior/reference/rvar.md) with the
same length as `x`, but where any entry equal to `1` in `dim(x)` has
