...
- rvar_args:
  ...
  allowed to accept
  [`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s as
  arguments. If `NULL` (the default), all arguments to `.f` are turned
  into arguments that accept
  [`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s, including
  arguments passed via `...` (if `rvar_dots` is `TRUE`).

- rvar_dots:

  (logical) Should dots (`...`) arguments also be converted? Only
  applies if `rvar_args` is `NULL` (i.e., all arguments are allowed to
  be [`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s).
...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s containing
...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s passed to it,
then returns a new
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md) representing
the output of those function evaluations. If the arguments contain no
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s, then `.f`
...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s (which should
be fast), using
