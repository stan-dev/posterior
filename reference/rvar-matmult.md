...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s and are
broadcasted across all draws of the
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md) argument.
Tensor multiplication is used to efficiently multiply matrices across
draws, so if either `x` or `y` is an
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md), `x %**% y`
will be much faster than `rdo(x %*% y)`.

In R \>= 4.3, you can also use `%*%` in place of `%**%` for matrix
multiplication of
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s. In R \< 4.3,
S3 classes cannot properly override `%*%`, so you must use `%**%` for
