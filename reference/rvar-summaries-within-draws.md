...
An [`rvar`](https://mc-stan.org/posterior/reference/rvar.md) of length 1
(for [`range()`](https://rdrr.io/r/base/range.html), length 2; for
[`quantile()`](https://rdrr.io/r/stats/quantile.html), length equal to
`length(probs)`) with the same number of draws as the input rvar(s)
...
[rvar-summaries-over-draws](https://mc-stan.org/posterior/reference/rvar-summaries-over-draws.md).
