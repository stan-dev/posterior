...
Returns an [`rvar`](https://mc-stan.org/posterior/reference/rvar.md)
obtained by applying a function to margins of an array or
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md). Acts like
[`apply()`](https://rdrr.io/r/base/apply.html), except that the function
supplied (`.f`) should return an
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md), and the final
result is always an
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md).
...
- .f:
  ...
  an [`rvar`](https://mc-stan.org/posterior/reference/rvar.md) and the
  dimensions of the result of `.f` applied to each margin of `.x` must
  be able to be broadcasted to a common shape (otherwise the resulting
  [`rvar`](https://mc-stan.org/posterior/reference/rvar.md) cannot be
...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md) of dimension
`d` after being broadcast to a common shape, then `rvar_apply()` returns
an [`rvar`](https://mc-stan.org/posterior/reference/rvar.md) of
...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s, and the
result is simplified into an
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md). Unlike
...
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md) broadcasting
rules to bind results together rather than using vector recycling.

If you wish to apply functions over
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md)s where the
result is not intended to be simplified into an
[`rvar`](https://mc-stan.org/posterior/reference/rvar.md), you can use
the standard [`apply()`](https://rdrr.io/r/base/apply.html),
...
[`as_rvar()`](https://mc-stan.org/posterior/reference/as_rvar.md) to
convert objects to `rvar`s. See
[`rdo()`](https://mc-stan.org/posterior/reference/rdo.md),
[`rfun()`](https://mc-stan.org/posterior/reference/rfun.md), and
[`rvar_rng()`](https://mc-stan.org/posterior/reference/rvar_rng.md) for
higher-level interfaces for creating `rvar`s.
