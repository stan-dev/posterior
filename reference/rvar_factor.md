...
- x:
  ...
  - An array where the first dimension represents draws from a
    ...
    [`rvar`](https://mc-stan.org/posterior/reference/rvar.md) will have
    dimension `dim(x)[-1]`; that is, everything except the first
...
- nchains:
  ...
  [`rvar`](https://mc-stan.org/posterior/reference/rvar.md), in which
  case the number of chains it has is used.
...
A subtype of [`rvar()`](https://mc-stan.org/posterior/reference/rvar.md)
that represents a (possibly multidimensional) sample of a
...
[`rvar()`](https://mc-stan.org/posterior/reference/rvar.md): it is
...
[`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) it will
automatically return an object with the classes `"rvar_factor"` or
`c("rvar_ordered", "rvar_factor")`.

See [`rvar()`](https://mc-stan.org/posterior/reference/rvar.md) for more
details on the internals of the random variable datatype.
