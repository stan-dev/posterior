...
supports multiple formats and easy conversion between them. Currently the 
...
the [vignette](rvar.html) dedicated to this new datatype for details.
...
consistency (e.g., no dropping of dimensions with one level) and additional
methods. The exception to this is the `draws_rvars` format, which behave
somewhat similarly to arrays but are really a unique data type. See the separate
vignette on the `rvar` and `draws_rvars` data types for details.

The draws for our example come as a `draws_array` object
...
we could of course also import the draws from other sources like common base R
objects.

#### Example: create draws_matrix from a matrix

In addition to converting other `draws` objects the `as_draws_matrix()` function
will convert a regular matrix to a `draws_matrix`.
...
`draws_matrix()` constructor function that can be used to construct a draws
...
Analogous functions exist for the other formats and are used similarly.


## Manipulating `draws` objects

There are many methods available for manipulating draws objects in various ways.
All of the methods demonstrated below, as well as other methods available in the
package, are all available for every draws format.
...
interface for this purpose via the `subset_draws()`. For example, here is the
...
It is also possible to standard subsetting syntax with `draws` objects. The 
following is equivalent to the use of `subset_draws()` with the array above. 
...
quantiles and standard deviations, an experimental new diagnostic called R*
(`rstar()`), and others. For a list of available diagnostics and links to their
individual help pages see `help("diagnostics", "posterior")`.

## Other methods

In addition to the methods demonstrated in this vignette, posterior has various
other methods available for draws objects. The following is a (potentially
incomplete) list of the other available methods.

|**Method**|**Description**|
|:----------|:---------------|
| `order_draws()` | Order `draws` objects according to iteration and chain number |
| `repair_draws()`| Repair indices of `draws` objects so that iterations chains,
and draws are continuously and consistently numbered |
...
| `weight_draws()`| Add weights to draws objects, with one weight per draw, for
use in subsequent weighting operations |
| `extract_variable()` | Extract a vector of draws of a single variable  |
| `extract_variable_matrix()` | Extract an iterations x chains matrix of of
draws of a single variable |
| `merge_chains()` | Merge chains of `draws` objects into a single chain. | 
| `split_chains()` | Split chains of `draws` objects by halving the number of
iterations per chain and doubling the number of chains.  |
