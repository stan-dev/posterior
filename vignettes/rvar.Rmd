---
title: "The Random Variable Datatype"
author: "Matthew Kay"
date: "`r Sys.Date()`"
output: 
  html_vignette:
...
The `rvar()` is inspired by the [rv](https://cran.r-project.org/package=rv)
...
and with distribution visualizations in the [ggdist](https://mjskay.github.io/ggdist/) package.
...
Manual construction and modification of `rvar`s in this way is not recommended
...
do; see **TODO: INSERT LINK** for an introduction to `draws` objects. One difference is that
`draws_rvars` counts variables differently, because it allows variables to be multidimensional.
For example, the `post` object above contains two variables, `mu` and `Sigma`:
...
## Expectations and summaries

The `E()` and `mean()` functions are aliases of each other, producing means
within each cell of an `rvar`. For example:

```{r E}
E(mu)
```

You can similarly obtain the `median()` or `variance()` of `rvar` cells:
...
with `E()` / `mean()` or with `Pr()` (which is purely notational sugar):
...
may find you need to adopt other strategies (like `rvar_r()`, described below;
...
### Evaluating random number generators with `rvar_r()`

`rvar_r()` is an alternative to `rfun()`/`rdo()` designed specifically to work
...
You can use any function with this interface with `rvar_r()`, and it will adapt
it to be able to take `rvar` arguments and return an `rvar`, as follows:

```{r rvar_r_ex}
mu <- rvar_r(rnorm, 4, mean = 1:4, sd = 1)
sigma <- rvar_r(rgamma, 1, shape = 1, rate = 1)
x <- rvar_r(rnorm, 4, mu, sigma)
x
```

In contrast to the `rfun()` and `rdo()` examples above, `rvar_r()` takes 
...
the data used to generate the predictions. `rvar`s can then be visualized
