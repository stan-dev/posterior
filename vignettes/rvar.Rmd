...
is also the basis of the `draws_rvar()` format.
...
## The `draws_rvar` datatype

The `draws_rvar()` datatype, like all `draws` datatypes in posterior, contains
...
converted into the `draws_rvar()` format. This dataset has 100 iterations from 
...
```{r post}
post <- as_draws_rvar(example_draws("multi_normal"))
post
```

The `draws_rvar()` datatype works much the same way that other `draws` formats
do; see **TODO: INSERT LINK** for an introduction to `draws` objects. One difference is that
`draws_rvar` counts variables differently, because it allows variables to be multidimensional.
For example, the `post` object above contains two variables, `mu` and `Sigma`:

```{r variables_draws_rvar}
variables(post)
```
